...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  try {
    ...
    const title = `${p.nom}, ${p.age} ans à ${p.ville?.nom || 'Québec'} – RendezVous Québec`
    const desc = `${p.tagline} — Découvrez le profil de ${p.nom}, ${p.age} ans, à ${p.ville?.nom || 'Québec'}. ${p.categorie?.nom || ''}. Profil vérifié sur RendezVous Québec.`
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.4fr)', gap: 40, alignItems: 'start', marginBottom: 60 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              {p.ville && <Link href={`/regions/${p.ville.slug.current}`} style={{ background: 'rgba(59,130,246,.08)', border: '1px solid rgba(59,130,246,.2)', color: '#93c5fd', fontSize: '.78rem', padding: '5px 12px', borderRadius: 50, fontWeight: 600, textDecoration: 'none' }}>🏙️ {p.ville.nom}</Link>}
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,.035)', border: '1px solid rgba(225,29,72,.2)', borderRadius: 18, padding: 22 }}>
              ...
              <p style={{ textAlign: 'center', marginTop: 9, color: '#3e444d', fontSize: '.73rem' }}>Vous serez redirigé·e vers une plateforme partenaire vérifiée</p>
            </div>
          </div>
        </div>
